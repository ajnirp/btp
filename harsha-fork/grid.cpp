...
Grid::Grid(float a_t, float a_cell_size,model_t a_m){
	...
	for(int i=0;i<w;i++){
		for (int j = 0; j < h; j++)
		{
			...
			if(curr->particle_present()) {
				curr->vel_y += alpha * (curr->temperature - temperature_air);
			}
			// 	curr->vel_y -= time_step * 9.8;
		}
	}
}
...
void Grid::temperature_solver()
{
	
}

float Grid::phi_at(int i, int j)
{
	if (0 <= i and i < w and 0 <= j and j < h)
		return cells[i][j]->phi;
	// TODO
}

void Grid::implicit_surface_solver()
{
	// TODO: Obtain w
	for (int i = 1 ; i < w-1 ; i++) {
		for (int j = 1 ; j < h-1 ; j++) {
			float phi_x = 0f;
			if (w1 > 0) phi_x = phi_at(i,j) - phi_at(i-1,j);
			else if (w1 < 0) phi_x = phi_at(i+1,j) - phi_at(i,j);
			float phi_y = 0f;
			if (w2 > 0) phi_y = phi_at(i,j) - phi_at(i,j-1);
			else if (w2 < 0) phi_y = phi_at(i,j+1) - phi_at(i,j);
			cells[i][j]->phi = phi - time_step * (w1*phi_x + w2*phi_y);
		}
	}
	// TODO: confirm that boundary cell values can be taken to be 0
	for (int i = 0 ; i < w ; i++) {
		cells[i][0]->phi = 0f;
		cells[i][h-1]->phi = 0f;
	}
